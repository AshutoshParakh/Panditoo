...
export function ReferralCampaignsPage() {
  ...
  useEffect(()=>{load();},[]);
  const totals=useMemo(()=>rows.reduce((a,r)=>({signups:a.signups+Number(r.signups||0),bookings:a.bookings+Number(r.paid_bookings||0),revenue:a.revenue+Number(r.revenue||0),commission:a.commission+Number(r.commission_earned||0)}),{signups:0,bookings:0,revenue:0,commission:0}),[rows]);
  const open=(row=null)=>{setEditing(row||{});form.resetFields();form.setFieldsValue(row?{...row,starts_at:row.starts_at?dayjs(row.starts_at):null,ends_at:row.ends_at?dayjs(row.ends_at):null}:{discount_type:null,campaign_cost:0,min_order_amount:0,commission_percent:3,is_active:true});};
  ...
  return <Space direction="vertical" size={18} className="page-stack">
    ...
    <Row gutter={14}><Col span={6}><Card><Statistic title="Attributed signups" value={totals.signups}/></Card></Col><Col span={6}><Card><Statistic title="Paid first bookings" value={totals.bookings}/></Card></Col><Col span={6}><Card><Statistic title="Attributed revenue" value={totals.revenue} prefix="₹" precision={0}/></Card></Col><Col span={6}><Card><Statistic title="Commission earned" value={totals.commission} prefix="₹" precision={0}/></Card></Col></Row>
    ...
    <Modal open={Boolean(editing)} onCancel={()=>setEditing(null)} onOk={()=>form.submit()} title={`${editing?.id?"Edit":"Create"} referral campaign`} width={820}><Form form={form} layout="vertical" onFinish={save}><Row gutter={16}>
      ...
      <Col span={8}><Form.Item name="partner_name" label="Partner / shop owner"><Input placeholder="Ashu / Temple shop"/></Form.Item></Col><Col span={8}><Form.Item name="partner_phone" label="Partner phone"><Input placeholder="9876543210"/></Form.Item></Col><Col span={8}><Form.Item name="commission_percent" label="Commission %" initialValue={3}><InputNumber min={0} max={100} style={{width:"100%"}}/></Form.Item></Col>

...
try{
  ...
  await page.locator(".phone-field input").fill("9876543210");const sendOtpButton=page.getByRole("button",{name:/Send secure OTP/i});check(await sendOtpButton.isDisabled(),"OTP request allowed without policy consent");await page.locator(".auth-consent input").check();await sendOtpButton.click();await page.locator("input.otp").fill("123456");await page.getByRole("button",{name:/Verify & continue/i}).click();
  await page.getByLabel(/Full name/i).fill("Ritika Sharma");await page.getByLabel(/^Email/i).fill("ritika@example.com");await page.getByLabel(/^Address/i).fill("Vijay Nagar, Indore");await page.getByRole("button",{name:/Create my account/i}).click();await page.waitForURL(`**/book/${poojaId}`);

...
export default function BookingsScreen({ navigation }) {
  ...
  const renderBooking = ({ item }) => {
    ...
    const otpDay = Number(item.service_otp?.service_days || 1) > 1 ? ` · DAY ${item.service_otp.visit_number}/${item.service_otp.service_days}` : "";
    const name = item.service_otp ? `OTP ${item.service_otp.code} · ${item.service_otp.phase === "start" ? "START" : "COMPLETE"}${otpDay}` : ceremonyName;
    const date = new Date(Number(item.service_days || 1) > 1 ? item.current_visit_date || item.booking_date : item.booking_date).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" });

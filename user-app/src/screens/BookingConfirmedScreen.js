...
const ServiceOtpBanner = ({ otp, hindi }) => {
  ...
  return <View style={s.otpCard}><View style={s.otpLiveBadge}><Text style={s.otpLiveText}>OTP READY</Text></View>{Number(otp.service_days || 1) > 1 ? <Text style={s.otpDay}>{hindi ? `दिन ${otp.visit_number} / ${otp.service_days}` : `DAY ${otp.visit_number} OF ${otp.service_days}`}</Text> : null}<Text style={s.otpLabel}>{otp.phase === "start" ? "START POOJA" : "COMPLETE POOJA"}</Text><Text selectable style={s.otpCode}>{otp.code}</Text><Text style={s.otpTimer}>{secondsLeft > 0 ? `Expires in ${minutes}:${seconds}` : "OTP expired"}</Text><Text style={s.otpHint}>{hindi ? "यह OTP केवल अपने पंडित जी को बताएं" : "Share this code only with your pandit"}</Text></View>;
};

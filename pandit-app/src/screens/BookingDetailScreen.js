...
export default function BookingDetailScreen({ route, navigation }) {
  ...
  const verifyServiceOtp = async () => {
    ...
    try {
      ...
      setBooking((current) => otpPhase === "start" ? { ...current, service_started_at: payload.service_started_at } : { ...current, booking_status: "completed", service_completed_at: payload.service_completed_at });
      setOtpPhase(null);
      setServiceOtp("");
      Alert.alert(otpPhase === "start" ? "Pooja started" : "Pooja completed", otpPhase === "start" ? "The verified service timer is now running." : "Completion verified successfully.");
  ...
  return (
    <><SafeAreaView style={s.screen}><ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
      ...
      {canComplete ? booking.service_started_at ? <><View style={s.dateNotice}><Text style={s.dateNoticeText}>Pooja in progress • verified timer running</Text></View><TouchableOpacity style={s.completeButton} onPress={() => requestServiceOtp("end")} disabled={otpLoading}><Text style={s.completeText}>{otpLoading ? "Sending OTP…" : "Finish Pooja with customer OTP"}</Text></TouchableOpacity></> : <TouchableOpacity style={s.completeButton} onPress={() => requestServiceOtp("start")} disabled={otpLoading}><Text style={s.completeText}>{otpLoading ? "Sending OTP…" : "Start Pooja with customer OTP"}</Text></TouchableOpacity> : <View style={s.dateNotice}><Text style={s.dateNoticeText}>Start Pooja will unlock on the scheduled ceremony date.</Text></View>}</>}
    </ScrollView></SafeAreaView>

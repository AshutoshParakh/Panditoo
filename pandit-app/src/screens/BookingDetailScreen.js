...
import { ActivityIndicator, Alert, Linking, Modal, Platform, SafeAreaView, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View } from "react-native";
...
class MapErrorBoundary extends React.Component {
  ...
  componentDidCatch(err) { console.warn("MapView error in BookingDetailScreen:", err); }
  render() {
...
export default function BookingDetailScreen({ route, navigation }) {
  ...
  const requestServiceOtp = async (phase) => {
    setOtpLoading(true);
    try {
  ...
  const verifyServiceOtp = async () => {
    if (!/^\d{6}$/.test(serviceOtp)) return Alert.alert("Invalid OTP", "Enter the 6-digit OTP shared by the customer.");
    setOtpLoading(true);
    try {
      const response = await fetch(`${API_URL}/bookings/${bookingId}/service/${otpPhase === "start" ? "start" : "end"}`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ otp: serviceOtp }) });
      const payload = await response.json();
      if (!response.ok || !payload.success) throw new Error(payload.message || "OTP verification failed");
      setOtpPhase(null);
      setServiceOtp("");
      await fetchBooking();
      if (otpPhase === "start") {
        Alert.alert(payload.service_days > 1 ? `Day ${payload.visit_number} started` : "Pooja started", "The verified service timer is now running.");
      } else if (payload.booking_completed === false) {
        const nextDate = new Date(payload.next_visit.scheduled_date).toLocaleDateString(hindi ? "hi-IN" : "en-IN", { weekday: "long", day: "2-digit", month: "short" });
        Alert.alert(`Day ${payload.completed_visit_number} complete`, `Return ${nextDate} at ${String(payload.next_visit.scheduled_time || "").slice(0, 5)} for day ${payload.next_visit.visit_number} of ${payload.service_days}.`);
      } else {
        Alert.alert("Pooja completed", payload.service_days > 1 ? `All ${payload.service_days} daily visits are verified and complete.` : "Completion verified successfully.");
      }
  ...
  const isBundle = Number(booking.service_days || 1) > 1;
  const pooja = (hindi ? booking.pooja_name_hi : booking.pooja_name_en) || booking.pooja_name_en || "Pooja ceremony";
  const activeServiceDate = isBundle ? booking.current_visit_date || booking.service_end_date : booking.booking_date;
  const date = new Date(activeServiceDate);
  const validDate = !Number.isNaN(date.getTime());
  const canComplete = validDate && new Date().setHours(0, 0, 0, 0) >= new Date(date).setHours(0, 0, 0, 0);
  const activeVisitStarted = isBundle ? booking.current_visit_started_at : booking.service_started_at;
  const rawLat = Number(booking.latitude);
  const rawLng = Number(booking.longitude);
  const coordinates = Number.isFinite(rawLat) && Number.isFinite(rawLng) && !isNaN(rawLat) && !isNaN(rawLng) ? { latitude: rawLat, longitude: rawLng } : null;
  ...
  return (
    <><SafeAreaView style={s.screen}><ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
      ...
      {isBundle && !pending ? <View style={s.bundleCard}><View style={s.bundleHead}><View><Text style={s.bundleEyebrow}>DAILY BUNDLE</Text><Text style={s.bundleTitle}>Day {booking.current_visit_number || booking.service_days} of {booking.service_days}</Text></View><Text style={s.bundleProgress}>{booking.completed_visits || 0}/{booking.service_days}</Text></View><Text style={s.bundleText}>{activeVisitStarted ? "Today’s pooja is in progress. Complete it with the customer OTP." : canComplete ? `Today’s visit is ready. Be at the customer address by ${String(booking.current_visit_time || booking.booking_time || "").slice(0, 5)}.` : `Your next visit is ${validDate ? date.toLocaleDateString(hindi ? "hi-IN" : "en-IN", { weekday: "long", day: "2-digit", month: "short" }) : "scheduled soon"} at ${String(booking.current_visit_time || booking.booking_time || "").slice(0, 5)}.`}</Text><View style={s.progressTrack}><View style={[s.progressFill, { width: `${Math.min(100, (Number(booking.completed_visits || 0) / Number(booking.service_days)) * 100)}%` }]} /></View><Text style={s.bundleReminder}>You must return every scheduled day until all {booking.service_days} visits are completed.</Text></View> : null}
      ...
      {!pending && <><View style={s.card}><Text style={s.cardLabel}>CEREMONY LOCATION</Text><Text style={s.address}>{booking.address || "Address will be shared before the ceremony."}</Text>{coordinates && <View style={s.mapWrap}><MapErrorBoundary fallback={null}><MapView style={s.map} initialRegion={{ ...coordinates, latitudeDelta: 0.012, longitudeDelta: 0.012 }} scrollEnabled={false} zoomEnabled={false}><Marker coordinate={coordinates} /></MapView></MapErrorBoundary></View>}<TouchableOpacity style={s.directionsButton} onPress={directions}><Text style={s.directionsText}>Open directions</Text></TouchableOpacity></View>
      ...
      {canComplete ? activeVisitStarted ? <><View style={s.dateNotice}><Text style={s.dateNoticeText}>{isBundle ? `Day ${booking.current_visit_number} in progress • verified timer running` : "Pooja in progress • verified timer running"}</Text></View><TouchableOpacity style={s.completeButton} onPress={() => requestServiceOtp("end")} disabled={otpLoading}><Text style={s.completeText}>{otpLoading ? "Sending OTP…" : isBundle ? `Complete day ${booking.current_visit_number} with OTP` : "Finish Pooja with customer OTP"}</Text></TouchableOpacity></> : <TouchableOpacity style={s.completeButton} onPress={() => requestServiceOtp("start")} disabled={otpLoading}><Text style={s.completeText}>{otpLoading ? "Sending OTP…" : isBundle ? `Start day ${booking.current_visit_number} with OTP` : "Start Pooja with customer OTP"}</Text></TouchableOpacity> : <View style={s.dateNotice}><Text style={s.dateNoticeText}>{isBundle ? `Day ${booking.current_visit_number} unlocks on ${validDate ? date.toLocaleDateString(hindi ? "hi-IN" : "en-IN", { weekday: "long", day: "2-digit", month: "short" }) : "its scheduled date"}. Return at the selected time.` : "Start Pooja will unlock on the scheduled ceremony date."}</Text></View>}</>}
    </ScrollView></SafeAreaView>
    <Modal visible={Boolean(otpPhase)} transparent animationType="fade" onRequestClose={() => setOtpPhase(null)}><View style={s.modalBackdrop}><View style={s.otpCard}><Text style={s.otpEyebrow}>{otpPhase === "start" ? "START VERIFICATION" : "COMPLETION VERIFICATION"}</Text><Text style={s.otpTitle}>Enter customer OTP</Text><Text style={s.otpHint}>A 6-digit OTP was sent to the customer’s registered mobile number.</Text><TextInput value={serviceOtp} onChangeText={(value) => setServiceOtp(value.replace(/\D/g, "").slice(0, 6))} keyboardType="number-pad" maxLength={6} autoFocus style={s.otpInput} placeholder="000000" placeholderTextColor="#B8ADA5" /><TouchableOpacity style={s.completeButton} onPress={verifyServiceOtp} disabled={otpLoading}><Text style={s.completeText}>{otpLoading ? "Verifying…" : otpPhase === "start" ? "Verify & start timer" : "Verify & complete"}</Text></TouchableOpacity><TouchableOpacity style={s.otpCancel} onPress={() => { setOtpPhase(null); setServiceOtp(""); }}><Text style={s.otpCancelText}>Cancel</Text></TouchableOpacity></View></View></Modal></>
  );
}
